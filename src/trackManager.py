...
class TrackManager:
    def __init__(self):
        self.chosen_model = YOLO("./models/yolov8s.pt")
        self.tracker = './models/bytetrack.yaml'
    ...
    def predict_and_detect(self, img, classes=[]):
        ...
        for result in results:
            for box in result.boxes:
                if result.names[int(box.cls[0])] == "person":
                    ...
                    if int(box.xyxy[0][1]) > self.exclusion_line_y or int(box.xyxy[0][0]) > self.exclusion_line_x:
                        continue
        ...
        cv2.putText(img, f"Personas: {num_people}", (10, 50),
